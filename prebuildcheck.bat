...
rem :: 3. Test: Use !SCRIPT_DIR! (safe), do NOT use %SCRIPT_DIR% or %~dp0 (unsafe)
if NOT "%1" == "silent" (
...
rem echo Running %lintexe% run !LINT_MOD_FLAG! on dnsbollocks only
rem "%lintexe%" --color always run !LINT_MOD_FLAG! .\internal\dnsbollocks\ .\cmd\dnsbollocks\
rem if errorlevel 1 goto :fail
...
rem if /i "%1" == "silent" goto skip_full_lint
